    ...
    return (
        hasRoomCode ? (<Dialog open={isOpen} onOpenChange={setIsOpen} >
            ...
            <DialogContent className="max-w-md h-fit border-2 border-gray-300 dark:border-white bg-white dark:bg-black">
                ...
                <form className="space-y-4 mt-4" onSubmit={(e) => { e.preventDefault(); setIsOpen(false); window.location.href = `http://localhost:3001/room/${ref.current?.value}`; }}>
        ...
        <DialogContent className="max-w-md h-fit border-2 border-gray-300 dark:border-white bg-white dark:bg-black">
            ...
            <form className="space-y-4 mt-4" onSubmit={(e) => { e.preventDefault(); setIsOpen(false); const generatedRoomId = Math.random().toString(36).substring(2, 8).toUpperCase(); window.location.href = `http://localhost:3001/room/${generatedRoomId}`; }}>
